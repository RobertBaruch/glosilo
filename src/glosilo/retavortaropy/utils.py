...
from glosilo.retavortaropy.data import vortaro


def find_uzos_with_kaps(
    element: vortaro.Element,
    ...
    def _traverse(current_element: vortaro.Element, current_kap: vortaro.Kap | None):
        ...
        if (
            isinstance(current_element, vortaro.HasKap)
            and current_element.kap is not None
        ):
...
def filter_uzos_by_fak(
    uzos_with_kaps: list[tuple[vortaro.Uzo, vortaro.Kap | None]], fak: str
) -> list[tuple[vortaro.Uzo, vortaro.Kap | None]]:
    """
